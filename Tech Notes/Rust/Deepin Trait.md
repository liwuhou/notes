...
你知道吗？在 Rust 中，许多的运算符都可以被重载，在这里运算符其实只是特征方法调用的语法糖而已。例如 `a + b` 里的 `+` 其实是 `std::ops::Add` 特征里的 `add` 方法的调用。因此我们可以很轻易的为自定义类型实现该特征来支持些类型的各种运算符操作。
...
这里变量 `c` 是存放了实现 `Pet` 特征的特征对象，可以看到第一种方式 `Vec<&impl Pet>` 的类型是不能通过编译的，Rust 会抛出错误，表明 `impl Trait` 的形式只能用于函数或方法，因为它其实是个特征泛型的[[Trait#使用特征来约束函数参数|语法糖]]。
