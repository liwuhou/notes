...
这里举的例子有点烂，我其实就是想说明这个主次关系。类型 `Dog` 想实现 `Pet` 之前，得先实现 `Animal` 特征，从实现中也可以看到 `Pet` 特征中调用了 `Animal` 特征的方法，如果只实现 `Pet`，而不实现 `Animal` 那就乱了套了。好在 Rust 有特征约，能约束类型的特征。这也有点像 `Pet` 扩展了 `Animal` 特征一样，这种行为也叫 `supertrait`。
