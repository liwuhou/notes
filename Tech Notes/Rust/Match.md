...
如果某个值被 `match` 语句匹配到了，那么该值就会被移动到匹配的分支中，如果想要在匹配的分支中使用该值，就需要使用引用来使用该值。
...
fn main() {
  let x = Some(Point {x: 0, y: 0});

  match x {
    Some(ref p) => println!("({}, {})", p.x, p.y), // 使用 ref 定义，表示这个匹配分支里，只使用值的引用
    _ => panic!("No match!"),
  }
  println!(x); // 这里 x  的所有权没有发生转移，可以接着使用
}
...
需要注意的是，`match` 的序列匹配**只允许数字或是字符**，因为要确保它们是连续的。而且只能用 `..=` 划分的闭区间，不能使用 `..` 这种半开区间。
